...
figure(10)
LEQ= sqrt(Ua.^2 + Va.^2);
M=csvread('Re20N30Y0.5.csv',1,0);
Ux=M(:,1);Uy=M(:,2);Uz=M(:,3);xa=M(:,6);ya=M(:,7);
Ubar=sqrt(Ux.^2 + Uy.^2);
plot(x,LEQ(:,16),xa,Ubar,Linewidth=2);
legend('Matlab','OpenFoam')
xlabel('location');ylabel('Speed');title('Velocity Profile at {l_y}=0.5'); 
set(gca,'fontsize',16);
%%
figure(11)
LEQ= sqrt(Ua.^2 + Va.^2);
M=csvread('Re20N30X0.5.csv',1,0);
Ux=M(:,1);Uy=M(:,2);Uz=M(:,3);xa=M(:,6);ya=M(:,7);
Ubar=sqrt(Ux.^2 + Uy.^2);
plot(x,LEQ(16,:),xa,Ubar,Linewidth=2);
legend('Matlab','OpenFoam')
xlabel('location');ylabel('Speed');title('Velocity Profile at {l_x}=0.5'); 
set(gca,'fontsize',16);
